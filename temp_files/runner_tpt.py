...
from .loops import  TPTLoop
from mmengine.runner import EpochBasedTrainLoop, IterBasedTrainLoop, TestLoop, ValLoop, EpochBasedTrainLoop, BaseLoop
...
class Runner_TPT(Runner):
    ...
    _tpt_loop: Optional[Union[BaseLoop, Dict]]
    ...
    def __init__(
        ...
        aug_dataloader: Optional[Union[DataLoader, Dict]] = None,
        aug_dataloader2: Optional[Union[DataLoader, Dict]] = None,
        aug_dataloader3: Optional[Union[DataLoader, Dict]] = None,
        aug_dataloader4: Optional[Union[DataLoader, Dict]] = None,
    ...
    def from_cfg(cls, cfg: ConfigType) -> 'Runner':
        ...
        runner = cls(
            ...
            aug_dataloader=cfg.get('aug_dataloader'),
            aug_dataloader2=cfg.get('aug_dataloader'),
            aug_dataloader3=cfg.get('aug_dataloader'),
            aug_dataloader4=cfg.get('aug_dataloader'),
    ...
    @property
    def tpt_loop(self):
        """:obj:`BaseLoop`: A loop to run testing-time prompt tuning."""
        if isinstance(self._tpt_loop, BaseLoop) or self._tpt_loop is None:
            # breakpoint()
            return self._tpt_loop
            
        else:
            # breakpoint()
            self._tpt_loop = self.build_tpt_loop(self._tpt_loop)
            return self._tpt_loop
    @property
    def train_dataloader(self):
        """The data loader for training."""
        return self.tpt_loop.dataloader
    ...
    def build_dataloader(self,dataloader: Union[DataLoader, Dict],
        ...
        # aug_dataloader_cfg = copy.deepcopy(aug_dataloader)

        # # build dataset
        # aug_dataset_cfg = aug_dataloader_cfg.pop('dataset')
        
        # if isinstance(dataset_cfg, dict):
        #     aug_dataset = DATASETS.build(aug_dataset_cfg)
        #     if hasattr(aug_dataset, 'full_init'):
        #         aug_dataset.full_init()
        # else:
        #     # fallback to raise error in dataloader
        #     # if `dataset_cfg` is not a valid type
        #     aug_dataset = aug_dataset_cfg
        # breakpoint()
        # dataset = torch.utils.data.TensorDataset(dataset, aug_dataset)
        ...
        if batch_sampler_cfg is None:
            ...
            if  batch_sampler_cfg['type']=='AspectRatioBatchSampler':
                ...
                # breakpoint()
            elif batch_sampler_cfg['type']=='AugAspectRatioBatchSampler':
                # breakpoint()
                batch_sampler = DATA_SAMPLERS.build(
                batch_sampler_cfg,
                default_args=dict(
    ...
    # def build_tpt_loop(self, loop: Union[BaseLoop, Dict]) -> BaseLoop: #base on build_test_loop
    #     """Build test loop.
    ...
    def build_tpt_loop(self, loop: Union[BaseLoop, Dict]) -> BaseLoop: #base on build_train_loop
        """Build training loop.
        ...
        if isinstance(loop, BaseLoop):
            ...
            raise TypeError(
                f'tpt_loop should be a Loop object or dict, but got {loop}')
        ...
        if 'type' in loop_cfg and 'by_epoch' in loop_cfg:
            raise RuntimeError(
                'Only one of `type` or `by_epoch` can exist in `loop_cfg`.')
        # breakpoint()
        # loop = TPTLoop(**loop_cfg, runner=self, dataloader=self._train_dataloader)
        
        loop = TPTLoop(**loop_cfg, runner=self, 
                        dataloader=self._train_dataloader,
                        aug_dataloader=self._aug_dataloader,
                        aug_dataloader2=self._aug_dataloader2,
                        aug_dataloader3=self._aug_dataloader3,
                        aug_dataloader4=self._aug_dataloader4)
        # breakpoint()
        # if 'type' in loop_cfg:
        #     loop = LOOPS.build(
        #         loop_cfg,
    ...
    def train_tpt(self) -> nn.Module:
        """Launch test-time prompt-tuning (training).
        ...
        if self._train_loop is None:
            raise RuntimeError(
                ...
                '`optimizer` and `param_scheduler` arguments when '
                'initializing runner.')
        if self._tpt_loop is None:
            raise RuntimeError(
                '`self._tpt_loop` should not be None when calling train '
                'method. Please provide `train_dataloader`, `train_cfg`, '
                '`optimizer` and `param_scheduler` arguments when '
                'initializing runner.')
        # breakpoint()
        self._tpt_loop = self.build_tpt_loop(
            self._tpt_loop)  # type: ignore
        ...
        self.optim_wrapper.initialize_count_status(
            self.model,
            self._tpt_loop.iter,  # type: ignore
            self._tpt_loop.max_iters)  # type: ignore

        # Maybe compile the model according to options in self.cfg.compile
        # This must be called **AFTER** model has been wrapped.
        self._maybe_compile('train_step')

        model = self.tpt_loop.run()  # type: ignore
        self.call_hook('after_run')
        return model
