...
from mmdet.evaluation import voc_classes, dwd_classes,cityscapes_classes
import cityscapesscripts.helpers.labels as CSLabels

# label_ids = {name: i for i, name in enumerate(voc_classes())}
# label_ids = CSLabels.id2label
# label_ids = {'unlabeled': 0, 'person': 24, 'rider': 25, 'car':26, 'truck': 27, 'bus':28, 'motor':32,'bike':33}
# label_ids = {'unlabeled': 0, 'person': 1, 'rider': 2, 'car':3, 'truck': 4, 'bus':5, 'motorcycle':6,'bicycle':7}
# dwd_classes = ['person','rider','car','truck','bus','motorcycle','bicycle']
label_ids = {name: i for i, name in enumerate(dwd_classes())}
def parse_xml(args):
...
def cvt_to_coco_json(annotations):
    ...
    for category_id, name in enumerate(dwd_classes()):
        # breakpoint()
        category_item = dict()
