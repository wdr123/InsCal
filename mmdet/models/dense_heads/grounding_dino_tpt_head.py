...
class GroundingDINOTPTHead(GroundingDINOHead):
    ...
    def loss(self, hidden_states: Tensor, references: List[Tensor],
             ...
             enc_outputs_class_aug: Tensor, enc_outputs_coord_aug: Tensor,
             enc_outputs_class_aug2: Tensor, enc_outputs_coord_aug2: Tensor,
             enc_outputs_class_aug3: Tensor, enc_outputs_coord_aug3: Tensor,
             enc_outputs_class_aug4: Tensor, enc_outputs_coord_aug4: Tensor,
        ...
        new_outputs = torch.stack([enc_outputs_class, enc_outputs_class_aug,enc_outputs_class_aug2,enc_outputs_class_aug3,enc_outputs_class_aug4]) 
        ...
        loss_inputs = outs + (enc_outputs_class, enc_outputs_coord,
                              enc_outputs_class_aug, enc_outputs_coord_aug,
                              enc_outputs_class_aug2, enc_outputs_coord_aug2,
                              enc_outputs_class_aug3, enc_outputs_coord_aug3,
                              enc_outputs_class_aug4, enc_outputs_coord_aug4,
    ...
    def loss_by_feat(
        ...
        enc_cls_scores_aug: Tensor,
        enc_bbox_preds_aug: Tensor,
        enc_cls_scores_aug2: Tensor,
        enc_bbox_preds_aug2: Tensor,
        enc_cls_scores_aug3: Tensor,
        enc_bbox_preds_aug3: Tensor,
        enc_cls_scores_aug4: Tensor,
        enc_bbox_preds_aug4: Tensor,
        ...
        if enc_cls_scores is not None:
            ...
            enc_loss_cls, enc_losses_bbox, enc_losses_iou = \
                self.loss_by_feat_single_multi(
                    enc_cls_scores, enc_bbox_preds,
                    enc_cls_scores_aug, enc_bbox_preds_aug,
                    enc_cls_scores_aug2, enc_bbox_preds_aug2,
                    enc_cls_scores_aug3, enc_bbox_preds_aug3,
                    enc_cls_scores_aug4, enc_bbox_preds_aug4,
    ...
    # loss_by_feat_single
    def loss_by_feat_single_multi(self, cls_scores: Tensor, bbox_preds: Tensor,
                            cls_scores_aug: Tensor, bbox_preds_aug: Tensor,
                            cls_scores_aug2: Tensor, bbox_preds_aug2: Tensor,
                            cls_scores_aug3: Tensor, bbox_preds_aug3: Tensor,
                            cls_scores_aug4: Tensor, bbox_preds_aug4: Tensor,
                            batch_gt_instances: InstanceList,
        ...
        # cls_scores_list_aug = [cls_scores_aug[i] for i in range(num_imgs)]
        # bbox_preds_list_aug = [bbox_preds_aug[i] for i in range(num_imgs)]
        with torch.no_grad():
            cls_reg_targets = self.get_targets(cls_scores_list,
                                               bbox_preds_list,
                                               batch_gt_instances,
                                               batch_img_metas)
            # cls_reg_targets_aug = self.get_targets(cls_scores_list_aug,
            #                                    bbox_preds_list_aug,
            #                                    batch_gt_instances,
            #                                    batch_img_metas)                                
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
         num_total_pos, num_total_neg) = cls_reg_targets
        # (labels_list_aug, label_weights_list_aug, bbox_targets_list_aug, bbox_weights_list_aug,
        #  num_total_pos, num_total_neg) = cls_reg_targets_aug
        ...
        cls_scores_aug = torch.masked_select(cls_scores_aug, text_mask).contiguous()
        cls_scores_aug2 = torch.masked_select(cls_scores_aug2, text_mask).contiguous()
        cls_scores_aug3 = torch.masked_select(cls_scores_aug3, text_mask).contiguous()
        cls_scores_aug4 = torch.masked_select(cls_scores_aug4, text_mask).contiguous()
        ...
        if isinstance(self.loss_cls, QualityFocalLoss):
            ...
            loss_cls = self.loss_cls(
                cls_scores,cls_scores_aug, cls_scores_aug2,cls_scores_aug3,cls_scores_aug4,
        ...
        # breakpoint()
        # bboxes_aug = bbox_preds_aug.reshape(-1, 4)
        bboxes_aug = bbox_cxcywh_to_xyxy(bbox_preds_aug.reshape(-1, 4)) * factors
        bboxes_aug2 = bbox_cxcywh_to_xyxy(bbox_preds_aug2.reshape(-1, 4)) * factors
        bboxes_aug3 = bbox_cxcywh_to_xyxy(bbox_preds_aug3.reshape(-1, 4)) * factors
        bboxes_aug4 = bbox_cxcywh_to_xyxy(bbox_preds_aug4.reshape(-1, 4)) * factors
        ...
        # loss_bbox_aug = self.loss_bbox(bboxes_aug, bbox_targets, bbox_weights, avg_factor=num_total_pos)
        # loss_bbox_aug2 = self.loss_bbox(bboxes_aug2, bbox_targets, bbox_weights, avg_factor=num_total_pos)
        # breakpoint()
        return loss_cls, loss_bbox, loss_iou
    

    def _loss_dn_single(self, dn_cls_scores: Tensor, dn_bbox_preds: Tensor,
                        # dn_cls_scores_aug: Tensor, dn_bbox_preds_aug: Tensor,
                        batch_gt_instances: InstanceList,
                        batch_img_metas: List[dict],
